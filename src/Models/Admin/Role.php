...
class Role extends Model //implements SluggableInterface
{
  ...
  */
  public function can($permission)
  {
    if (is_array($permission)) {
      return (!! (count($permission) == count(array_intersect($this->permissions()->pluck('name')->toarray(), $permission))));
    } else {
  ...
  */
  ...
  {
    return (!! (count(array_intersect($this->permissions()->pluck('name')->toarray(), $permission)) > 0));
  }
}
